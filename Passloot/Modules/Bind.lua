...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"Bind", {}}}
		if (Rule.Bind) then Table[1][2][1] = {Rule.Bind, false} end
		return Table
	end
	if (FromVersion == 2) then
		local Table = {{"Bind", {}}}
		if (type(Rule.Bind) == "table") then for Key, Value in ipairs(Rule.Bind) do Table[1][2][Key] = {Value, false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	Widget.Info = {L["Bind On"], L["Selected rule will only match these items."]}
	return Widget
end
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("Bind", Data) end
	return Data
end
...
function module:GetBindSlotText(BindID)
	for Key, Value in ipairs(self.Choices) do if (Value.Value == BindID) then return Value.Name end end
	return ""
end
