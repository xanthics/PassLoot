...
module.Choices = {L["Any"], L["Group"], L["Raid"]}
...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"GroupRaid", {}}}
		if (Rule.GroupRaid) then Table[1][2][1] = {Rule.GroupRaid, false} end
		return Table
	end
	if (FromVersion == 2) then
		local Table = {{"GroupRaid", {}}}
		if (type(Rule.GroupRaid) == "table") then for Key, Value in ipairs(Rule.GroupRaid) do Table[1][2][Key] = {Value, false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	Widget.Info = {L["Group / Raid"], L["Selected rule will only match if you are in a group or raid."]}
	return Widget
end
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("GroupRaid", Data) end
	return Data
end
