...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"LearnedItem", {}}}
		if (type(Rule.LearnedItem) == "table") then for Key, Value in ipairs(Rule.LearnedItem) do Table[1][2][Key] = {Value, false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	Widget.Info = {L["Learned Item"], L["Selected rule will only match these items."]}
	return Widget
end
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("LearnedItem", Data) end
	return Data
end
...
function module:GetLearnedItemText(Which)
	for Key, Value in ipairs(self.Choices) do if (Value.Value == Which) then return Value.Name end end
	return ""
end
