...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"RequiredLevel", {}}}
		if (type(Rule.RequiredLevel) == "table") then for Key, Value in ipairs(Rule.RequiredLevel) do Table[1][2][Key] = {Value[1], Value[2], false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	DropDown.Info = {L["Required Level"], L["Selected rule will only match items when comparing the required level to this."]}
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("RequiredLevel", Data) end
	return Data
end
...
function module.Widget:DisplayWidget(Index)
	...
	if (select(4, GetBuildInfo()) < 30000) then
		UIDropDownMenu_SetText(module:GetRequiredLevelText(Value_LogicalOperator, Value_Comparison), module.Widget)
	else
...
function module.Widget:Evaluate(Logic)
	self.Environment.level = UnitLevel("player")
...
function module:DropDown_OnClick(Frame)
	...
	if (select(4, GetBuildInfo()) < 30000) then
		UIDropDownMenu_SetText(self:GetRequiredLevelText(LogicalOperator, Comparison), Frame.owner)
	else
