...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"Type", nil}, {"SubType", nil}, {"TypeSubType", {}}}
		if (Rule.Type and Rule.SubType) then Table[3][2][1] = {Rule.Type, Rule.SubType, false} end
		return Table
	end
	if (FromVersion == 2) then
		local Table = {{"Type", nil}, {"SubType", nil}, {"TypeSubType", {}}}
		if (type(Rule.TypeSubType) == "table") then for Key, Value in ipairs(Rule.TypeSubType) do Table[3][2][Key] = {Value[1], Value[2], false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	Widget.Info = {L["Type / SubType"], L["Selected rule will only match items with this type and subtype."]}
	return Widget
end
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("TypeSubType", Data) end
	return Data
end
...
function module.Widget:AddNewFilter()
	...
	local NewTable = {module.NewFilterValue_Type, module.NewFilterValue_SubType, false}
...
function module.Widget:SetMatch(ItemLink, Tooltip)
	...
	if (ItemType) then
		...
		if (module.CurrentTypeMatch == -1) then module:Debug("Could not find ItemType: " .. ItemType) end
	end
	if (ItemSubType) then
		module:Debug("Sub Type: " .. ItemSubType .. " Found: (" .. module.CurrentSubTypeMatch .. ") ")
		if (module.CurrentSubTypeMatch == -1) then module:Debug("Could not find ItemSubType: " .. ItemSubType) end
	end
end
