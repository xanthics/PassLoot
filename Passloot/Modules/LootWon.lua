...
module.ProfileOptionsTable = {
	...
	["type"] = "toggle",
	["get"] = function(info) return module:GetProfileVariable("ResetLootCounter") end,
...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"LootWonLogicalOperator", nil}, {"LootWonComparison", {}}}
		if (Rule.LootWonLogicalOperator and Rule.LootWonComparison) then Table[2][2][1] = {Rule.LootWonLogicalOperator, Rule.LootWonComparison, false} end
		return Table
	end
	if (FromVersion == 2) then
		local Table = {{"LootWonLogicalOperator", Rule.LootWonLogicalOperator}, {"LootWonComparison", {}}}
		if (type(Rule.LootWonComparison) == "table") then for Key, Value in ipairs(Rule.LootWonComparison) do Table[2][2][Key] = {Value[1], Value[2], false} end end
		return Table
	end
	return
end
...
function module:CHAT_MSG_LOOT(Event, ...)
	...
	if (#self.ItemsBeingRolledOn == 0) then return end
...
function module:RAID_ROSTER_UPDATE()
	if (UnitInRaid("player") or GetNumPartyMembers() > 0) then
		if (not self.InRaid and self:GetProfileVariable("ResetLootCounter")) then self:ResetProfileLootWonCounters() end
		self.InRaid = true
	else
		self.InRaid = false
	end
...
function module:PassLoot_OnRoll(Event, ItemLink, RuleNum, RollID, RollMethod) table.insert(self.ItemsBeingRolledOn, {ItemLink, RuleNum}) end
...
function module:CreateWidget_LootWonCounter()
	...
	EditBox.Info = {L["Loot Won Counter"], L["Loot Won Counter_Desc"]}
	return EditBox
end
...
function module.WidgetCounter:AddNewFilter() module:SetConfigOption("LootWonCounter", module.NewFilterValue_Counter) end

function module.WidgetCounter:RemoveFilter(Index)
	module:SetConfigOption("LootWonCounter", nil)
	module:SetConfigOption("LootWonComparison", {})
	-- module:SetConfigOption("LootWonLogicalOperator", {})
end
...
function module.WidgetCounter:IsException(RuleNum, Index) return false end

function module.WidgetCounter:SetException(RuleNum, Index, Value) end

function module.WidgetCounter:SetMatch(ItemLink, Tooltip) end

function module.WidgetCounter:GetMatch(RuleNum, Index) return true end
...
function module:CreateWidget_LootWonComparison()
	...
	DropDown.Info = {L["Loot Won Comparison"], L["Selected rule will only match items when comparing the loot won counter to this."]}
...
function module.WidgetComparison:GetData(RuleNum)
	...
	for Key, Value in ipairs(Data) do
		if (type(Value) ~= "table" or type(Value[1]) ~= "number" or type(Value[2]) ~= "number") then
			Data[Key] = {module.NewFilterValue_LogicalOperator, module.NewFilterValue_Comparison, false}
			Changed = true
		end
	end
	if (Changed) then module:SetConfigOption("LootWonComparison", Data) end
	return Data
end
...
function module.WidgetComparison:AddNewFilter()
	local Value = self:GetData()
	local NewTable = {module.NewFilterValue_LogicalOperator, module.NewFilterValue_Comparison, false}
	table.insert(Value, NewTable)
	module:SetConfigOption("LootWonComparison", Value)
	local Value = module.WidgetCounter:GetData()
	if (not Value) then module:SetConfigOption("LootWonCounter", module.NewFilterValue_Counter) end
end
...
function module.WidgetComparison:SetMatch(ItemLink, Tooltip) end

function module.WidgetComparison:GetMatch(RuleNum, Index)
	local Value = self:GetData(RuleNum)
	local LogicalOperator = Value[Index][1]
	local Comparison = Value[Index][2]
	local Counter = module.WidgetCounter:GetData(RuleNum)
...
function module:GetLootWonText(LogicalOperator, Comparison)
	for Key, Value in ipairs(self.Choices) do if (Value.Value == LogicalOperator) then return string.gsub(Value.Text, "%%num%%", Comparison) end end
end
