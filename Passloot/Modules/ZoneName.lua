...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"Zone", {}}}
		if (Rule.Zone) then Table[1][2][1] = {Rule.Zone, false} end
		return Table
	end
	if (FromVersion == 2) then
		local Table = {{"Zone", {}}}
		if (type(Rule.Zone) == "table") then for Key, Value in ipairs(Rule.Zone) do Table[1][2][Key] = {Value, false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	Widget.Info = {L["Zone Name"], L["Zone Name_Desc"]}
	return Widget
end
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("Zone", Data) end
	return Data
end
