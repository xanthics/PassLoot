...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"ItemLevel", {}}}
		if (type(Rule.ItemLevel) == "table") then for Key, Value in ipairs(Rule.ItemLevel) do Table[1][2][Key] = {Value[1], Value[2], false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	DropDown.Info = {L["Item Level"], L["Selected rule will only match items when comparing the item level to this."]}
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("ItemLevel", Data) end
	return Data
end
