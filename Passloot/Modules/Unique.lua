...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"Unique", {}}}
		if (Rule.Unique) then Table[1][2][1] = {Rule.Unique, false} end
		return Table
	end
	if (FromVersion == 2) then
		local Table = {{"Unique", {}}}
		if (type(Rule.Unique) == "table") then for Key, Value in ipairs(Rule.Unique) do Table[1][2][Key] = {Value, false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	Widget.Info = {L["Unique"], L["Unique_Desc"]}
	return Widget
end
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("Unique", Data) end
	return Data
end
...
function module:GetUniqueSlotText(UniqueID)
	for Key, Value in ipairs(self.Choices) do if (Value.Value == UniqueID) then return Value.Name end end
	return ""
end
