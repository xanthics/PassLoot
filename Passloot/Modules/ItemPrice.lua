...
function module:OnEnable()
	if (select(4, GetBuildInfo()) < 30200) then
		...
		if (not GetSellValue) then
			PassLoot:Print("GetSellValue() or ItemPrice-1.1 library not available")
			return
		end
	end
...
function module:OnDisable()
	if (select(4, GetBuildInfo()) < 30200) then if (not GetSellValue) then return end end
...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"ItemPrice", {}}}
		if (type(Rule.ItemPrice) == "table") then for Key, Value in ipairs(Rule.ItemPrice) do Table[1][2][Key] = {Value[1], Value[2], false} end end
		return Table
	end
	return
end
...
local function SetMoney_MoneyInputFrame(Frame, Money)
	...
	if (Gold == 0 and Silver == 0 and Copper == 0) then Gold, Silver, Copper = "", "", "" end
...
function module:CreateMoneyInputFrame()
	...
	Frame.Copper:SetScript("OnEnterPressed", function() Frame.Copper:ClearFocus() end)
...
function module:CreateWidget()
	...
	Frame.Info = {L["Item Price"], L["Selected rule will only match items when compared to vendor value."]}
	Frame:SetParent(nil)
	Frame:Hide()
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("ItemPrice", Data) end
	return Data
end
...
function module:SetMoney()
	if (not self.FilterIndex) then return end
	local Value = self.Widget:GetData()
...
function module:GetItemPriceText(LogicalOperator, Comparison)
	for Key, Value in ipairs(self.Choices) do if (Value.Value == LogicalOperator) then return string.gsub(Value.Text, "%%num%%", GetCoinTextureString(Comparison)) end end
end

function module:GetDropDownText(LogicalOperator) for Key, Value in ipairs(self.Choices) do if (Value.Value == LogicalOperator) then return Value.Name end end end
