...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"ZoneType", {}}}
		if (Rule.ZoneType) then Table[1][2][1] = {Rule.ZoneType, false} end
		return Table
	end
	if (FromVersion == 2) then
		local Table = {{"ZoneType", {}}}
		if (type(Rule.ZoneType) == "table") then for Key, Value in ipairs(Rule.ZoneType) do Table[1][2][Key] = {Value, false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	Widget.Info = {L["Zone Type"], L["Selected rule will only match items when you are in this type of zone."]}
	return Widget
end
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("ZoneType", Data) end
	return Data
end
	...
	function module.Widget:GetMatch(RuleNum, Index)
		...
		if (LookupTable[RuleValue[Index][1]][1] == "ANY" or LookupTable[RuleValue[Index][1]][1] == InInstance) then
			if (LookupTable[RuleValue[Index][1]][2] == "ANY" or LookupTable[RuleValue[Index][1]][2] == InstanceType) then
				if (LookupTable[RuleValue[Index][1]][3] == "ANY" or LookupTable[RuleValue[Index][1]][3] == Difficulty) then return true end
			end
		end
...
elseif (select(4, GetBuildInfo()) < 30300) then
	...
	function module.Widget:GetMatch(RuleNum, Index)
		...
		-- However, if we are IN the instance, this should correspond to the level of the instance.
		-- Both functions return the same values.
		if (self.LookupTable[RuleValue[Index][1]](InInstance, InstanceType, DungeonDifficulty, RaidDifficulty)) then return true end
		module:Debug("ZoneType doesn't match")
		return false
	end
else
	module.Widget.LookupTable = {
		...
		[7] = function(InInstance, InstanceType, Difficulty, MaxNumPlayers, DynamicDifficulty)
			return (InInstance == 1 and InstanceType == "raid" and MaxNumPlayers == 10 and Difficulty == 1 and DynamicDifficulty == 0)
		end,
		[9] = function(InInstance, InstanceType, Difficulty, MaxNumPlayers, DynamicDifficulty)
			return (InInstance == 1 and InstanceType == "raid" and MaxNumPlayers == 10 and ((Difficulty == 3 and DynamicDifficulty == 0) or (Difficulty == 1 and DynamicDifficulty == 1)))
		end,
		...
		[8] = function(InInstance, InstanceType, Difficulty, MaxNumPlayers, DynamicDifficulty)
			return (InInstance == 1 and InstanceType == "raid" and MaxNumPlayers == 25 and Difficulty == 2 and DynamicDifficulty == 0)
		end,
		[11] = function(InInstance, InstanceType, Difficulty, MaxNumPlayers, DynamicDifficulty)
			return (InInstance == 1 and InstanceType == "raid" and MaxNumPlayers == 25 and ((Difficulty == 4 and DynamicDifficulty == 0) or (Difficulty == 2 and DynamicDifficulty == 1)))
		end,
	}
	...
	function module.Widget:GetMatch(RuleNum, Index)
		...
		if (self.LookupTable[RuleValue[Index][1]](InInstance, InstanceType, Difficulty, MaxNumPlayers, DynamicDifficulty)) then return true end
		module:Debug("ZoneType doesn't match")
		return false
	end
end
...
function module:GetZoneTypeText(ZoneID)
	for Key, Value in ipairs(self.Choices) do
		if (#Value.Group > 0) then
			...
			if (Value.Value == ZoneID) then return Value.Name end
		end
	end
	return ""
end
