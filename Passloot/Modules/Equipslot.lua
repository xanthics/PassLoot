...
if (not LibStub("LibBabble-Inventory-3.0", true)) then return end
...
function module:SetupValues()
	...
	module.Choices = {{
		...
		["Group"] = {{
			...
			["Type"] = {"INVTYPE_RANGED", "INVTYPE_RANGEDRIGHT", "INVTYPE_THROWN"},
			["Value"] = 18,
		}, {
			["Name"] = BI["Two-Hand"] or INVTYPE_2HWEAPON, -- Two-Handed Weapon
			["Type"] = {"INVTYPE_2HWEAPON"},
			["Value"] = 25,
		}},
	}, {
		["Name"] = L["Accessories"],
...
function module:UpgradeDatabase(FromVersion, Rule)
	if (FromVersion == 1) then
		local Table = {{"EquipSlot", {}}}
		if (Rule.EquipSlot) then Table[1][2][1] = {Rule.EquipSlot, false} end
		return Table
	end
	if (FromVersion == 2) then
		local Table = {{"EquipSlot", {}}}
		if (type(Rule.EquipSlot) == "table") then for Key, Value in ipairs(Rule.EquipSlot) do Table[1][2][Key] = {Value, false} end end
		return Table
	end
	return
end
...
function module:CreateWidget()
	...
	Widget.Info = {L["Equip Slot"], L["Selected rule will only match items with this equip slot."]}
	return Widget
end
...
function module.Widget:GetData(RuleNum)
	...
	if (Changed) then module:SetConfigOption("EquipSlot", Data) end
	return Data
end
...
function module.Widget:SetMatch(ItemLink, Tooltip)
	...
	if (EquipSlot) then
		...
		if (module.CurrentMatch == -1) then module:Debug("Could not find EquipSlot: " .. (EquipSlot or "nil")) end
	end
end

function module.Widget:GetMatch(RuleNum, Index)
	local RuleValue = self:GetData(RuleNum)
	if (RuleValue[Index][1] > 1) then if (RuleValue[Index][1] ~= module.CurrentMatch) then return false end end
	return true
end
...
function module:GetEquipSlotText(EquipID)
	for Key, Value in ipairs(self.Choices) do
		if (#Value.Group > 0) then
			for GroupKey, GroupValue in ipairs(Value.Group) do if (GroupValue.Value == EquipID) then return GroupValue.Name end end
		else
			if (Value.Value == EquipID) then return Value.Name end
		end
	end
	return ""
end
...
function module:FindEquipSlot(Slot)
	for Key, Value in pairs(self.Choices) do
		if (#Value.Group > 0) then
			for GroupKey, GroupValue in pairs(Value.Group) do for TypeKey, TypeValue in pairs(GroupValue.Type) do if (Slot == TypeValue) then return GroupValue.Value end end end
		else
