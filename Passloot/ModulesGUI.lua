...
function PassLoot.Prototypes:RegisterDefaultVariables(RuleVariables)
	...
	if (type(RuleVariables) ~= "table") then return end
	for NewKey, NewValue in pairs(RuleVariables) do
		if (type(NewValue) ~= "table") then return end
		for VariableKey, VariableValue in pairs(PassLoot.DefaultTemplate) do if (NewValue[1] == VariableValue[1]) then return end end
	end
...
function PassLoot.Prototypes:AddWidget(Widget)
	if (type(Widget) ~= "table" or not Widget.GetNumFilters or not Widget.AddNewFilter or not Widget.RemoveFilter or not Widget.DisplayWidget or not Widget.GetFilterText or not Widget.SetMatch or
		not Widget.GetMatch or type(Widget.Info) ~= "table") then
	...
	for Key, Value in pairs(PassLoot.RuleWidgets) do if (Value == Widget) then return end end
	local Module = self:GetName()
	...
	for Key, Value in pairs(PassLoot.PluginInfo[Module].RuleWidgets) do if (Value == Widget) then return end end
...
function PassLoot.Prototypes:AddModuleOptionTable(TableName, Table)
	local Module = self:GetName()
	if (not PassLoot.OptionsTable.args.Modules.args[Module].args[TableName]) then PassLoot.OptionsTable.args.Modules.args[Module].args[TableName] = Table end
end

function PassLoot.Prototypes:RemoveModuleOptionTable(TableName, Table)
	local Module = self:GetName()
	if (PassLoot.OptionsTable.args.Modules.args[Module].args[TableName]) then PassLoot.OptionsTable.args.Modules.args[Module].args[TableName] = nil end
end
...
function PassLoot.Prototypes:SetConfigOption(Variable, Value, RuleNum)
	...
	if (RuleNum > 0 and Module and PassLoot.PluginInfo[Module] and PassLoot.PluginInfo[Module].RuleVariables) then
...
function PassLoot.Prototypes:GetConfigOption(Variable, RuleNum)
	RuleNum = RuleNum or PassLoot.CurrentRule
	if (RuleNum > 0) then return PassLoot.db.profile.Rules[RuleNum][Variable] end
end

function PassLoot.Prototypes:SetGlobalVariable(Variable, Value)
	local Module = self:GetName()
	if (Module and PassLoot.db.global.Modules and PassLoot.db.global.Modules[Module]) then
...
function PassLoot.Prototypes:GetGlobalVariable(Variable)
	local Module = self:GetName()
	if (Module and PassLoot.db.global.Modules and PassLoot.db.global.Modules[Module] and PassLoot.db.global.Modules[Module].Vars) then return PassLoot.db.global.Modules[Module].Vars[Variable] end
end

function PassLoot.Prototypes:SetProfileVariable(Variable, Value)
	local Module = self:GetName()
	if (Module and PassLoot.db.profile.Modules and PassLoot.db.profile.Modules[Module]) then
...
function PassLoot.Prototypes:GetProfileVariable(Variable)
	local Module = self:GetName()
	if (Module and PassLoot.db.profile.Modules and PassLoot.db.profile.Modules[Module] and PassLoot.db.profile.Modules[Module].ProfileVars) then
...
function PassLoot.Prototypes:CheckDBVersion(ModuleVersion, CallbackFunc)
	local Module = self:GetName()
	if (Module and PassLoot.PluginInfo[Module] and PassLoot.PluginInfo[Module].RuleVariables and ModuleVersion and (type(CallbackFunc) == "string" or type(CallbackFunc) == "function")) then
		...
		if (Version >= ModuleVersion) then return end
					...
					for RuleKey, RuleValue in ipairs(ProfileValue.Rules) do
						...
						if (ReturnData and type(ReturnData) == "table") then
							for ReturnKey, ReturnValue in pairs(ReturnData) do
								if (type(ReturnValue) == "table" and ReturnValue[1] and VariableList[ReturnValue[1]]) then PassLootDB.profiles[ProfileKey].Rules[RuleKey][ReturnValue[1]] = ReturnValue[2] end
		...
		elseif (self.db and self.db.profile and self.db.profile.Rules) then
			for RuleKey, RuleValue in ipairs(self.db.profile.Rules) do
				...
				if (ReturnData and type(ReturnData) == "table") then
					for ReturnKey, ReturnValue in pairs(ReturnData) do
						if (type(ReturnValue) == "table" and ReturnValue[1] and VariableList[ReturnValue[1]]) then self.db.profile.Rules[RuleKey][ReturnValue[1]] = ReturnValue[2] end
					end -- ReturnKey, ReturnValue
				end
			end -- RuleKey, RuleValue
		end
...
function PassLoot:IsModuleEnabled(Info) return self.modules[Info.arg].enabledState end
...
function PassLoot:TempIsException() return false end

function PassLoot:TempSetException() end
